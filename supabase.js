...
function startPresenceTracking(page) {
  updatePresence(page);
  clearInterval(_presenceInterval);
  _presenceInterval = setInterval(() => updatePresence(page), 30000);
}
...
async function clearPresence() {
  ...
  try {
    const { data: { user } } = await sb.auth.getUser();
    if (user) await sb.from('user_presence').delete().eq('user_id', user.id);
  } catch(_) {}
}
